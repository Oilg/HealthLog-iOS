...
struct RiskDetailView: View {
    ...
    private var info: RiskInfo { RiskInfo.info(for: risk.type) }
            ...
            if risk.severity != "high" {
                HStack(alignment: .top, spacing: 8) {
                    Image(systemName: "info.circle")
                        ...
                        .foregroundStyle(.secondary)
                        .padding(.top, 1)
                    Text("«\(risk.severityLabel)» — уровень потенциального вреда, а не вероятность. Уверенность показывает, насколько алгоритм уверен в обнаружении паттерна.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
